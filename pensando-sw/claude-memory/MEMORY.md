...
## Findings
- [local-IP-check HW multiplane validated (kenya-perf3/4)](project_local_ip_check_hw_multiplane.md) — PR#120693 positive path PASS on back-to-back 4x200G (v4 777/1515, v6 1504 Gb/s, miss=0 → per-plane learn+tm_iport+v6-key correct). Drop path NOT testable on back-to-back (sender eats meta-roce 2766 frames→0 wire arrivals); accept DOL+gtest. Reusable: Meta-RoCE UDP=**2766** not 4791; post-flash needs `nicctl update multiplane`; "device configuration (0)" warning cosmetic (puec/mrc name)
...
- [8x100G RL generic-peek fix — built, PARKED](project_rl_8x100_genpeek_parked.md) — collapses 4 per-port peek tables into 1 rotating generic table so RL recovery works for ports 4-7; branch pradeept/meter_rl_llc_8port (2 commits), artifacts in /vol/systest/agentq/rl-8x100-pradeept/; deprioritized 2026-09-09, pending kenya HW validation
- [RL byte-accurate + tawk fix, HOLB verified a-119](project_rl_byteaccurate_holb_verified.md) — kenya 4x200G path-1 9QP: RL off 548 → RL-on 766 Gbps (+40%). Byte-accurate meter needs window-lg2 **10** (not 4) + rate **195G** (not 191.5G) or it over-throttles→QP stall. err95 root cause: PIC_RL missing from tawk.c. Perftest reads old puec_nports (driver=mrc_nports)→binary-patch

## Tools
- [eth_dbgtool memzero — host-side HBM zero](reference_eth_dbgtool_memzero.md) — `eth_dbgtool memzero <hbm_addr> <len>` zeroes HBM directly from x86 host (no reflash/reset); abs region addr from build `gen/mem.json` "start". Reset debug counters in atomic regions surgically
...
- [AI-7522 reproduces on IPv4 = multi-QP bidir BW stall, NOT IPv6](project_ai7522_ipv4_multiqp.md) — SMC a-86: 1QP lat PASS, QP≥2 bidir BW FAIL (send_bw @16B match, spec_failure/ACK-gen-broken); QP=1 clean rules out IPv6 & SMC QoS; IPv6 direct compare blocked by Micas switch ND broken. Root cause = exact_cwnd_enforce (disable = workaround)
- [AI-7522 retx-ring off-by-one race theory](project_ai7522_retx_race_theory.md) — tx_s3 exact_cwnd_enforce: retx_pi++ (unconditional :588) vs snd_max snapshot (:87) non-atomic → phantom ring slot → WQE[retx_ci]=snd_una-1 → retransmit re-sends dup, hole never fills. Proven via ring dump; disable exact_cwnd = no repro
...
- [Firmware-armed p4e catch-all breaks non-AQ gtest → arm lazily](feedback_firmware_armed_table_nonaq_gtest.md) — eager always-on catch-all at check_init mass-fails non-AQ (learn never fires→all miss→drop) while AQ/HW pass; fix=install catch-all on FIRST learn (one-shot), fail-open until then. rdma_drop's en_drop_test bit is the stage-1 analog. non-AQ excludes p4pd libs (no host-side TCAM install). commit c6494bd149a
