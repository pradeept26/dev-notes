...
**BYTE-CONFIRMED root cause = host IPv6 routing (netplan), not a NIC bug (2026-07-28):** Read the QP's AH/header-template dst MAC directly from HBM via `eth_dbgtool memrd <sqcb2.header_template_addr << HDR_TEMP_ADDR_SHIFT(3)> 16` (or `memrd <addr> 16 3`). Result on SMC2 benic1: IPv4 loopback QP AH dst MAC = 80:ba:16:00:fa:95 (the SWITCH, non-own) -> ud_loopback=0 -> wire path works. IPv6 loopback QP AH dst MAC = 04:90:81:8c:7e:88 = the NIC's OWN MAC (dst==src==own, a self-addressed frame) -> ud_loopback=1 -> fails. Cause: `/etc/netplan/75-backend-pf-v4.yaml` gives each NIC source-policy routing (ip rule `from 30.2.N.1 lookup 10N`, table 10N = `default via 30.2.N.2` = switch) so RoCE resolves same-host peers via the switch (non-own MAC); `/etc/netplan/90-benic-ipv6.yaml` has NO policy routing (only inter-node routes 2001:db8:1-8 via ::2), so same-host IPv6 peers fall to the kernel local table -> dev lo -> own MAC. **Fix = host config: mirror the IPv4 source-policy routing for IPv6** (`from 2001:db8:N::1 -> table 20N: default via 2001:db8:N::2`). Then IPv6 self-writes get the switch MAC -> ud_loopback=0 -> wire path -> works, NO firmware change. (`ip route get` shows both peers as "local" because it uses default rule order; RoCE resolution honors the source-policy table for v4.)

Tooling notes: `eth_dbgtool memrd <addr> <size> [<shift>]` reads HBM. AH HBM PA = `sqcb2.header_template_addr << 3`. AH starts with the Eth header (dst MAC = first 6 bytes). AH entry = 96B (`AT_ENTRY_SIZE_BYTES`), `ah_pa = ah_base + ah_handle*96`. tcpdump does NOT see RoCE (kernel bypass); use mputrace `conf_pcap`/`dump_pcap` or switch SPAN for wire capture, but AH read is more direct for dmac.
