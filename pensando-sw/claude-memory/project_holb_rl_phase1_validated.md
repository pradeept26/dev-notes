...
- **Genuine no-connect edges (NOT RL, NOT tuning):** QP33+1-survivor (path-1 strands ~25/33 QPs on dead ports) and 512QP+port-shut. The 512+shut is a **multiplane perftest limit**: box has ONE rdma dev (rocep195s0f3, base GID 19.0.0.2) + IP-based `--planes` (19.1-4.0.2). At 512 QP with <4 planes (via port-shut OR listing only 3), base plane dest-GID → loopback ("Overriding destination gid to loopback: 19.0.0.2") → fast exit. 512 full-4-plane = 1509 ok; 512 3-plane no-shut also FAILS (proves it's plane-count, not admin-down); 9QP+shut works (low QP tolerates down plane). Not measurable via multiplane perftest. Neither edge affects RL conclusion — path-4 self-heal under port loss already shown at QP9 (859→1135) & QP63.
