#!/bin/bash
# watch_qp_status.sh — Watch top-level status of an RDMA Queue Pair.
#
# Shows congestion and active path counts at the QP level, complementing
# watch_qp_path_status.sh which drills into per-path congestion windows.
#
# Usage:
#   ./watch_qp_status.sh <qp_id> [OPTIONS]
#   ./watch_qp_status.sh -q <qp_id> [OPTIONS]
#
# Options:
...
# ── Watch ─────────────────────────────────────────────────────────────────────
# Fields: congestion state | active path count
watch -n "$INTERVAL" \
    "sudo nicctl show rdma queue-pair \
        --queue-pair-id \"$QP\" \
        --lif \"$LIF\" \
        --status \
    | grep -i 'congestion\|path'"
